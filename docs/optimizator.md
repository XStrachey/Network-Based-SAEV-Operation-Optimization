...
| **服务奖励**    | 奖励        | `svc_gate`（唯一容量弧）                              | $\displaystyle \color{#090}{\text{coef\_svc\_gate}} = -\,\mathrm{VOT}\cdot w_{ijt}$                                      | 由 06：默认 $w_{ijt}=$`cfg.costs_equity.service_weight_default`（默认15.7）；可被 `cfg.unmet_weights_overrides[t][(i,j)]` 覆盖；受 `cfg.flags.enable_service_reward` 控制 | `svc_gate` 的容量 = $D_{ijt}$（当期需求）；奖励以**负成本**落在 gate 上。                                                                                          |
